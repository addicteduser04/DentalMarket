...
export function StudentPackCard({pack,locale="fr"}:{pack:StudentPack;locale?:Locale}){
  ...
  return <Link href={`/student-packs/${pack.universities?.slug}/${pack.slug}`} className="card group overflow-hidden">
    <div className="relative aspect-[4/3] bg-white/[.04]">{pack.image_url?<Image src={pack.image_url} fill unoptimized={shouldBypassImageOptimization(pack.image_url)} alt={pack.name} className="object-cover transition duration-500 group-hover:scale-105"/>:<div className="grid h-full place-items-center text-cyan-300/45">{<BrandLogo compact inverted/>}</div>}<span className="status-success absolute right-3 top-3 rounded-full px-3 py-1 text-xs font-bold backdrop-blur">{t("available")}</span></div>
