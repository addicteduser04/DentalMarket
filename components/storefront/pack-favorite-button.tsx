...
export function PackFavoriteButton({packId,locale="fr"}:{packId:string;locale?:Locale}){
 ...
 async function toggle(){if(!userId){router.push("/account");return}const db=createClient();if(active)await db.from("student_pack_favorites").delete().eq("user_id",userId).eq("pack_id",packId);else await db.from("student_pack_favorites").insert({user_id:userId,pack_id:packId});setActive(!active)}
