...
export function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-ink/10 bg-cream/90 backdrop-blur-xl">
    <div className="container-shell flex h-20 items-center justify-between">
      <Link href="/" aria-label="DENTALNOVA — Accueil"><BrandLogo compact/></Link>
    ...
    {open&&<nav className="container-shell grid gap-1 border-t border-ink/10 py-4 text-sm font-semibold md:hidden">{[["/","Accueil"],["/search","Catalogue"],["/account","Mon compte"]].map(([h,l])=><Link onClick={()=>setOpen(false)} className="rounded-xl px-3 py-3 hover:bg-white" href={h} key={h}>{l}</Link>)}</nav>}
  </header>
}
