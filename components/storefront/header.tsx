...
export function Header({locale="fr"}:{locale?:Locale}) {
  ...
  return <header className={`z-50 w-full border-b border-white/10 text-white backdrop-blur-xl transition-colors ${home ? "absolute left-0 top-0 bg-[#05090d]/25" : "sticky top-0 bg-[#070c11]/90"}`}>
    ...
    {open&&<nav className="container-shell grid gap-1 border-t border-white/10 bg-[#070c11]/95 py-4 text-sm font-semibold md:hidden">{[["/",t("home")],["/search",t("catalogue")],["/student-packs",t("studentPacks")],["/account",t("account")]].map(([h,l])=><Link onClick={()=>setOpen(false)} className="rounded-xl px-3 py-3 hover:bg-white/10" href={h} key={h}>{l}</Link>)}</nav>}
  </header>
}
