...
export function Header({locale="fr"}:{locale?:Locale}) {
  ...
  return <header className={`z-50 w-full border-b border-white/10 text-white backdrop-blur-xl transition-colors ${home ? "absolute left-0 top-0 bg-[var(--dn-header-transparent)]" : "sticky top-0 bg-[var(--dn-header-solid)]"}`}>
    <div className="container-shell flex h-20 items-center justify-between">
      ...
      <nav className="hidden items-center gap-7 text-sm font-semibold lg:flex"><Link href="/">{t("home")}</Link><Link href="/search">{t("catalogue")}</Link><Link href="/student-packs">{t("studentPacks")}</Link><Link href="/search">{t("search")}</Link><Link href="/account">{t("account")}</Link></nav>
      <div className="flex items-center gap-2">
        ...
        <button aria-label={open ? t("closeMenu") : t("openMenu")} className="grid h-10 w-10 place-items-center lg:hidden" onClick={()=>setOpen(!open)}>{open?<X/>:<Menu/>}</button>
      </div>
    </div>
    {open&&<nav className="container-shell grid gap-1 border-t border-white/10 bg-[var(--dn-header-menu)] py-4 text-sm font-semibold lg:hidden">{[["/",t("home")],["/search",t("catalogue")],["/student-packs",t("studentPacks")],["/account",t("account")]].map(([h,l])=><Link onClick={()=>setOpen(false)} className="rounded-xl px-3 py-3 hover:bg-white/10" href={h} key={h}>{l}</Link>)}<div className="mt-2 flex items-center gap-2 border-t border-white/10 px-3 pt-3 sm:hidden"><LocaleSwitcher locale={locale}/><ThemeToggle/></div></nav>}
  </header>
}
