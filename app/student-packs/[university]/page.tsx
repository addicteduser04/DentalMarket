...
export default async function UniversityPacks({params,searchParams}:{params:{university:string};searchParams:{year?:string}}){
  const {universities,years,packs,recommendations}=await getStudentPackCatalog(),university=universities.find(x=>x.slug===params.university),locale=getLocale(),t=(key:Parameters<typeof translate>[1])=>translate(locale,key);if(!university)notFound();
  ...
  const supplies=recommendations.filter(item=>item.university_id===university.id&&(!searchParams.year||item.academic_years?.code===searchParams.year));
