...
export default async function StudentPacksPage(){
  const {universities,packs}=await getStudentPackCatalog(),locale=getLocale(),t=(key:Parameters<typeof translate>[1])=>translate(locale,key);
  return <div className="store-page"><div className="container-shell py-14"><p className="eyebrow">DENTANOVA</p><h1 className="display mt-3 text-5xl">{t("studentPacks")}</h1><p className="mt-4 max-w-2xl text-white/55">{t("packsIntro")}</p>
