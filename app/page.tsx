...
export default async function Home(){
  ...
  return <div className="home-cinematic bg-[#070c11] text-white">
    <HomeVideoHero/>
    <section className="border-y border-white/10 bg-[#091018]">
    ...
    {featuredCategories.length>0&&<section className="py-16 md:py-24"><div className="container-shell"><div><p className="eyebrow">{t("catalogue")}</p><h2 className="display mt-3 text-4xl md:text-6xl">{t("featuredCategories")}</h2></div><div className="category-market-grid mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{featuredCategories.map((category,index)=><Link key={category.id} href={`/category/${category.slug}`} className="category-visual-card group relative min-h-[320px] overflow-hidden rounded-[1.5rem]"><Image src={categoryVisuals[index%categoryVisuals.length]} alt="" fill sizes="(max-width: 640px) 100vw, 25vw" className="object-cover transition duration-700 group-hover:scale-105"/><span className="category-card-shade absolute inset-0"/><span className="absolute inset-x-5 bottom-5 flex items-end justify-between gap-3"><b className="text-2xl">{category.name}</b><i className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/10"><DirectionalArrow size={17}/></i></span></Link>)}</div></div></section>}
    ...
    <section className="pb-24 pt-14"><div className="container-shell"><div className="relative overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-[radial-gradient(circle_at_80%_20%,rgba(55,171,255,.24),transparent_35%),linear-gradient(135deg,#0d1922,#081018)] px-7 py-12 md:px-14 md:py-16"><Sparkles className="text-cyan-300"/><h2 className="display mt-5 max-w-3xl text-4xl md:text-6xl">{t("finalStudentCta")}</h2><p className="mt-5 max-w-2xl text-white/55">{t("finalStudentCtaBody")}</p><Link href="/student-packs" className="button mt-8">{t("discoverPacks")}<DirectionalArrow size={18}/></Link></div></div></section>
