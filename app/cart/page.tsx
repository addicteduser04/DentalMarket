...
export default function CartPage() {
  ...
  useEffect(()=>{
    setLocale(document.documentElement.lang==="ar"?"ar":"fr");
      ...
      <div className="grid gap-3">
        {items.map(item => <div key={item.key} className="card flex flex-wrap items-center gap-3 p-4 sm:flex-nowrap sm:gap-4">
          ...
          <div className="ml-auto flex items-center rounded-full border border-white/15 bg-white/[.04]">
            <button aria-label="Réduire" className="grid h-10 w-10 place-items-center" onClick={() => setQuantity(item.key,item.quantity-1)}><Minus size={14}/></button>
            <span className="w-7 text-center text-sm font-bold">{item.quantity}</span>
            <button aria-label="Augmenter" className="grid h-10 w-10 place-items-center" onClick={() => setQuantity(item.key,item.quantity+1)}><Plus size={14}/></button>
          </div>
          <button aria-label="Supprimer" onClick={() => remove(item.key)} className="grid h-10 w-10 place-items-center text-white/35 hover:text-red-300"><Trash2 size={18}/></button>
        </div>)}
      </div>
