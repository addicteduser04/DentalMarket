...
export default async function EditOffer({params}:{params:Promise<{id:string}>}){
  const db=createClient(),{id}=await params,[offerResult,categoriesResult,productsResult]=await Promise.all([
    db.from("offers").select("*").eq("id",id).single(),db.from("categories").select("*").order("display_order"),db.from("products").select("*").order("name"),
  ]);
