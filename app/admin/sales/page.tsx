...
export default async function SalesPage({searchParams}:{searchParams:Promise<Params>}){
  ...
  if(filters.from)query=query.gte("sale_at",`${filters.from}T00:00:00`);
  if(filters.to)query=query.lte("sale_at",`${filters.to}T23:59:59.999`);
  if(filters.payment)query=query.eq("payment_status",filters.payment);
  if(filters.delivery)query=query.eq("delivery_status",filters.delivery);
  ...
  const rows=((data||[]) as unknown as SaleRecord[]).filter(sale=>
    ...
    (!filters.product||sale.sale_items?.some(item=>item.product_id===filters.product))
  );
  return <>
    ...
    <form className="card mt-7 grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-4">
      <label className="admin-field xl:col-span-2">Client ou référence<input name="q" defaultValue={filters.q} placeholder="Nom, téléphone ou référence"/></label>
      <label className="admin-field">Du<input type="date" name="from" defaultValue={filters.from}/></label><label className="admin-field">Au<input type="date" name="to" defaultValue={filters.to}/></label>
      <label className="admin-field">Produit<select name="product" defaultValue={filters.product}><option value="">Tous</option>{products?.map(product=><option key={product.id} value={product.id}>{product.name}</option>)}</select></label>
      <Filter name="payment" label="Paiement" value={filters.payment} options={paymentStatusLabels}/><Filter name="delivery" label="Livraison" value={filters.delivery} options={deliveryStatusLabels}/><Filter name="status" label="Vente" value={filters.status} options={saleStatusLabels}/>
