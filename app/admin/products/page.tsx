...
export default async function ProductsPage({searchParams}:{searchParams:{q?:string;stock?:string}}){
  const db=createClient();
  const [{data:products},{data:categories}]=await Promise.all([
    db.from("products").select("*").order("updated_at",{ascending:false}),
    db.from("categories").select("*").order("display_order"),
  ]);
  const allProducts=products||[],allCategories=categories||[],query=(searchParams.q||"").toLowerCase();
  const rows=allProducts.filter(product=>
    (!query||product.name.toLowerCase().includes(query))&&
    (!searchParams.stock||product.stock_status===searchParams.stock)
  );
  return <>
    ...
    <form className="mt-7 flex flex-col gap-3 sm:flex-row"><input name="q" defaultValue={searchParams.q} className="field" placeholder="Rechercher un produit"/><select name="stock" className="field max-w-48" defaultValue={searchParams.stock}><option value="">Tous les stocks</option><option value="in_stock">En stock</option><option value="on_order">Sur commande</option><option value="out_of_stock">Épuisé</option></select><button className="button">Filtrer</button></form>
