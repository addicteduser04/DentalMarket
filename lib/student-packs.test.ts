...
import {activePackPrice,packAvailability,packCartKey,packComponentAvailability,packSavings,type StudentPack} from "./student-packs";
...
describe("student pack commerce",()=>{
 ...
 it("calculates savings only from normal and promotional pack prices",()=>{expect(packSavings(pack({manual_price:100,promotional_price:80,component_total:500}))).toEqual({amount:20,percentage:20});expect(packSavings(pack({manual_price:100,promotional_price:null,component_total:500}))).toBeNull();expect(packSavings(pack({manual_price:100,promotional_price:120}))).toBeNull()});
 ...
 it("uses canonical product availability for every pack component",()=>{expect(packComponentAvailability({id:"c",pack_id:"p1",product_id:"x",variation_id:"v1",quantity:1,is_required:true,display_order:0,products:{id:"x",name:"Fraise",slug:"fraise",description:null,price:10,compare_at_price:null,category_id:null,images:[],stock_status:"in_stock",target_audience:"both",availability_status:"in_stock",variations:[{id:"v1",label:"012",price:10,availability:"out_of_stock"}],is_active:true,is_featured:false}})).toBe("in_stock")});
 ...
 it("includes selected optional products in the handoff",()=>{const message=buildWhatsAppMessage([{itemType:"student_pack",name:"Pack clinique",componentSummary:["1× Miroir"],optionalComponentSummary:["2× Gants"],quantity:1,price:220}]);expect(message).toContain("Options : 2× Gants");expect(message).toContain("220.00 MAD")});
});
