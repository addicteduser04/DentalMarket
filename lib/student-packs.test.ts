...
describe("student pack commerce",()=>{
 ...
 it("calculates maximum complete required packs",()=>{const value=pack({student_pack_components:[{id:"c",pack_id:"p1",product_id:"x",quantity:2,is_required:true,display_order:0,products:{id:"x",name:"X",slug:"x",price:10,images:[],is_active:true,stock_status:"in_stock",stock_tracking:true,stock_quantity:5} as any}]});expect(packAvailability(value)).toEqual({status:"in_stock",quantity:2})});
 it("fails closed for missing or unavailable required products",()=>{expect(packAvailability(pack({student_pack_components:[{id:"c",pack_id:"p1",product_id:"x",quantity:1,is_required:true,display_order:0}]})).status).toBe("out_of_stock")});
