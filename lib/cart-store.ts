...
export type CartItem = { key:string; productId:string; slug:string; name:string; image?:string; variationLabel?:string; quantity:number; price:number };
...
export const useCart = create<CartState>()(persist((set) => ({
  items: [],
  add: item => set(s => { const key = `${item.productId}:${item.variationLabel ?? ""}`; const found=s.items.find(x=>x.key===key); return {items:found?s.items.map(x=>x.key===key?{...x,quantity:x.quantity+1}:x):[...s.items,{...item,key,quantity:1}]}; }),
