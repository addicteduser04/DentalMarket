...
describe("administrator product management",()=>{
  ...
  it("loads the exact product and handles missing records",()=>{expect(detail).toContain("{id}=await params");expect(detail).toContain('.eq("id",id)');expect(detail).toContain("if(!product)notFound()")});
